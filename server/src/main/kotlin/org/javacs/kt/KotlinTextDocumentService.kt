...
class KotlinTextDocumentService(
    ...
    private fun doLint(cancelCallback: () -> Boolean) {
        ...
        val compilerMessages = LoggingMessageCollector.drain()
        if (!cancelCallback.invoke()) {
            reportDiagnostics(files, context.diagnostics, compilerMessages)
        }
    }
    ...
    private fun reportDiagnostics(compiled: Collection<URI>, kotlinDiagnostics: Diagnostics, compilerMessages: List<CompilerMessageEntry>) {
        ...
        val byFile = combinedDiagnostics.groupBy({ it.first }, { it.second })
